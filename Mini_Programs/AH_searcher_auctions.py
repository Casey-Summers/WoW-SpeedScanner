...
def main():
    ...
    for item in matches:
        g_price = item['buyout'] // 10000 if item['buyout'] else "BID ONLY"
        suffix_str = f" ({item['suffix']})" if item['suffix'] else ""
        print(f"- {item['quantity']} available for {g_price}g{suffix_str}")
